		...
		if ( $#t2w ) then
			...
			if ( ! $useold ||  ! -e ${t2wimg}.4dfp.img ) then
				...
				while ( $i <= $#t2w )
					if (! $E4dfp) then
						if ( $doBIDS == 1)
							ln -sf $inpath/study${t2w[$i]}/study${t2w[$i]}.nii.gz .
							cp $inpath/study${t2w[$i]}/study${t2w[$i]}.json .
							gunzip -f study${t2w[$i]}.nii.gz
						else	 
...
if ( $distort == 1 ) then		# spin echo distortion correction
	...
	while ( $i <= $#sefm )
		...
		while ( $j <= $#study ) 
			if ( $doBIDS == 1 ) then
				ln -sf $inpath/study$study[$j]/study$study[$j].nii.gz SEFM/${patid}_sefm_Grp${i}_${j}.nii.gz
				cp $inpath/study$study[$j]/study$study[$j].json SEFM/${patid}_sefm_Grp${i}_${j}.json
				gunzip -f ./SEFM/${patid}_sefm_Grp${i}_${j}.nii.gz
			else	 
...
########################################################
			set SEFMstr = ( $SEFMstr -i ${patid}_sefm_Grp${i}_${j} $SEFMped $readout_time_sec )
			@ j++
		end
		pushd SEFM
...
while ($k <= $runs)
	...
	pushd bold$run
		if (! $E4dfp && ! -e $patid"_b"$run.4dfp.img ) then
			if ( $doBIDS == 1 ) then
				ln -sf $inpath/study$BOLDruns[$k]/study$BOLDruns[$k].nii.gz ./$patid"_b"$run.nii.gz
				cp $inpath/study$BOLDruns[$k]/study$BOLDruns[$k].json ./$patid"_b"$run.json
				gunzip -f ./$patid"_b"$run.nii.gz
			else
				dcm2niix -o . -f $patid"_b"$run -z n -w 1 $inpath/study$BOLDruns[$k] || exit $status
			endif
			nifti_4dfp -4 $patid"_b"$run $patid"_b"$run -N || exit $status
#######################################################################################
...
#######################################################################################
			if ( $GetBoldConfig ) then	# BIDS2params.awk generates run-specific params file from json:
				...
				gawk -f $RELEASE/BIDS2params.awk $patid"_b"$run.json > $patid"_b"$run.params || exit $status
				set pedindex = `grep pedindex $patid"_b"$run.params | gawk '{print $NF}'`
				echo "fslhd $patid"_b"${run}.nii | gawk -f $RELEASE/GetPED_2019.awk PEDindex=$pedindex"
				set ped = `fslhd $patid"_b"${run}.nii | gawk -f $RELEASE/GetPED_2019.awk PEDindex=$pedindex`
