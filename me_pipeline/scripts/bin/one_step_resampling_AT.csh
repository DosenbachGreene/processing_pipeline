#!/bin/csh -f
#$Header: /data/petsun4/data1/solaris/csh_scripts/RCS/one_step_resampling_AT.csh,v 1.18 2023/03/21 22:25:07 tanenbauma Exp $
#$Log: one_step_resampling_AT.csh,v $
#Revision 1.18  2023/03/21 22:25:07  tanenbauma
#Added ability to not specify a reference space keeping it in the same space.
#
...
set rcsid = '$Id: one_step_resampling_AT.csh,v 1.18 2023/03/21 22:25:07 tanenbauma Exp $'
echo $rcsid
date
...
if ( $warpmode == 1 ) then
	...
	set STRresample = "$STRresample none"
endif
...
while ($i < $dim4)	# loop on frame
	...
	echo -n " $j"
	if ( $Ncores > 1 ) then
		sem -P $Ncores Resampling.csh $STRresample ${padded} $j $epi
	else

		Resampling.csh $STRresample ${padded} $j $epi || goto FAILED
	endif
	@ k = 1
	while ( $k <= $#epi )
