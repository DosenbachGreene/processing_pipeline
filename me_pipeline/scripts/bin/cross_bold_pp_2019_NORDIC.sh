...
if ( $distort == 1 ) then               # spin echo distortion correction
        ...
        while ( $i <= $#sefm )
                ...
                while ( $j <= $#study ) 
                        ...
                        set pedindex = \
                                `gawk '$1 =="\"PhaseEncodingDirection\":" { sub(/,/,"",$NF); gsub(/\"/,"",$NF);print $NF}' \
                                SEFM/${patid}_sefm_Grp${i}_${j}.json`   # $pedindex is +/-{i j k} index of PE direction
                        #set SEFMped = `fslhd SEFM/${patid}_sefm_Grp${i}_${j}.nii | gawk -f $RELEASE/GetPED_2019.awk PEDindex=$pedindex`
                        set readout_time_sec = `gawk '$1 =="\"TotalReadoutTime\":" { sub(/,/,"",$NF);print $NF}' \
                                                        SEFM/${patid}_sefm_Grp${i}_${j}.json`
####################################################
