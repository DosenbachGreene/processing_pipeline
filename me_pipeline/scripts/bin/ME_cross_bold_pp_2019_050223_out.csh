#!/bin/csh -f
#$Header: /data/petsun4/data1/solaris/csh_scripts/RCS/ME_cross_bold_pp_2019.csh,v 1.10 2022/12/16 20:54:12 avi Exp $
...
set idstr = '$Id: ME_cross_bold_pp_2019.csh,v 1.10 2022/12/16 20:54:12 avi Exp $'
echo $idstr
...
@ k = 1
while ($k <= $runs)
	echo bold$runID[$k]/$patid"_b"$runID[$k]_faln >> ${patid}"_b"${runID[$k]}"_faln_xr3d".lst
	@ i = 1
	while ($i <= $necho)
		echo bold$runID[$k]/$patid"_b"$runID[$k]_echo${i}_faln >> $patid"_b"$runID[$k]"_faln_xr3d".lst
		@ i++
	end
	
echo	cross_realign3d_4dfp -n$skip -qv$normode -l${patid}"_b"${runID[$k]}_faln_xr3d.lst 
	cross_realign3d_4dfp -n$skip -qv$normode -l${patid}"_b"${runID[$k]}_faln_xr3d.lst >> ${patid}_faln_xr3d.log
	@ k++
end	

@ k = 1	
while ($k <= $runs)
	mat2dat bold$runID[$k]/${patid}_b${runID[$k]}_faln_xr3d.mat -RD -n$skip || exit $status
	/bin/mv bold$runID[$k]/${patid}_b${runID[$k]}_faln_xr3d.*dat movement
	@ k++
end
...
######################################
	grep -x -A4 "t4 frame 1" bold${run}/$patid"_b"${run}_faln_xr3d.mat | tail -4 >  $adir/bold${run}_frame.mat
############################
# tmp intensity scale factor
############################
	grep -x -A6 "t4 frame 1" bold${run}/$patid"_b"${run}_faln_xr3d.mat | tail -1 >> $adir/bold${run}_frame.mat
##################################################################
# convert cross_realign3d_4dfp first frame affine xform to t4_file
##################################################################
	aff_conv x4 bold${run}/$patid"_b"${run}_faln bold${run}/$patid"_b"${run}_faln $adir/bold${run}_frame.mat \
				bold${run}/$patid"_b"${run}_faln bold${run}/$patid"_b"${run}_faln \
				$adir/${anat}_to_${anat}_faln_xr3d_t4 || exit $status
##############################################
# xform reciprocal bias field onto first frame
##############################################
	t4_inv $adir/${anat}_to_${anat}_faln_xr3d_t4 $adir/${anat}_faln_xr3d_to_${anat}_t4  || exit $status
	if (1) then
		set anatstr = ${run}_faln
	else
		set anatstr = ${run}_echo1_faln
	endif
	if ($BiasField) then
		t4img_4dfp $adir/${anat}_faln_xr3d_to_${anat}_t4 bold${run}/${patid}"_b"${run}_invBF \
			   $adir/bold${run}_invBF_on_frame1 -Obold${run}/$patid"_b"${run}_faln || exit $status
####################################################
...
####################################################
	...
	t4_mul     $adir/${anat}_faln_xr3d_to_${anat}_t4 $adir/${anat}_uwrp_to_${target:t}_t4 \
		   $adir/${anat}_faln_xr3d_to_${target:t}_t4 || exit -1
	t4img_4dfp $adir/${anat}_to_${anat}_faln_xr3d_t4 ${PHA_on_EPI} ${PHA_on_EPI}_faln_xr3d -O${PHA_on_EPI}
...
###########################################################################
		t4_mul $adir/${anat}_faln_xr3d_to_${anat}_t4 $adir/${anat}_uwrp_to_${struct:t}_t4 \
		       $adir/${anat}_faln_xr3d_to_${struct:t}_t4 || exit $status
		aff_conv 4f $adir/${anat}_uwrp atlas/${struct:t} $adir/${anat}_faln_xr3d_to_${struct:t}_t4 \
			    $adir/${anat}_uwrp atlas/${struct:t} $adir/${anat}_faln_xr3d_to_${struct:t}.mat
		convertwarp --ref=$outspace --premat=$adir/${anat}_faln_xr3d_to_${struct:t}.mat --warp1=$warp --postmat=$postmat \
			--out=$adir/${anat}_faln_xr3d_to_fn_MNI152_T1_2mm_to_${outspace:t}_fnirt_coeff
############################################################
# generate command for one_step_resampling_AT.csh = $strwarp
############################################################
		set strwarp = "-postwarp $adir/${anat}_faln_xr3d_to_fn_MNI152_T1_2mm_to_${outspace:t}_fnirt_coeff"
############################
# transform anat to outspace
############################
		fugue --loadfmap=${PHA_on_EPI} --dwell=$dwell --unwarpdir=$ped --saveshift=${PHA_on_EPI}_shiftmap || exit $status
		t4_mul $adir/${anat}_to_${anat}_faln_xr3d_t4 $adir/${anat}_faln_xr3d_to_${struct:t}_t4 $adir/${anat}_to_${struct:t}_t4 || exit $status
		...
		aff_conv 4f $adir/${anat}_uwrp $target   $adir/${anat}_faln_xr3d_to_${target:t}_t4 \
			    $adir/${anat}_uwrp $target:t $adir/${anat}_faln_xr3d_to_${target:t}.mat || exit $status
		convert_xfm -omat $adir/${anat}_faln_xr3d_to_${outspace:t}.mat \
			-concat $postmat $adir/${anat}_faln_xr3d_to_${target:t}.mat || exit $status
		set strwarp = "-postmat $adir/${anat}_faln_xr3d_to_${outspace:t}.mat"
############################
# transform anat to outspace
############################
		fugue --loadfmap=${PHA_on_EPI} --dwell=$dwell --unwarpdir=$ped --saveshift=${PHA_on_EPI}_shiftmap || exit $status
		aff_conv 4f $adir/${anat} $adir/${anat} $adir/${anat}_to_${anat}_faln_xr3d_t4 \
			    $adir/${anat} $adir/${anat} $adir/${anat}_to_${anat}_faln_xr3d.mat || exit $status
		convertwarp --ref=$outspace --shiftmap=${PHA_on_EPI}_shiftmap --shiftdir=$ped --premat=$adir/${anat}_to_${anat}_faln_xr3d.mat \
			--postmat=$adir/${anat}_faln_xr3d_to_${outspace:t}.mat --out=$adir/${anat}_to_${outspace:t}_warp || exit $status
		applywarp --ref=$outspace --in=$adir/${anat} --warp=$adir/${anat}_to_${outspace:t}_warp \
			--out=$adir/${anat}_uwrp_on_${outspacestr} || exit $status
	endif
...
if (0) then	# one_step_resampling_AT.csh alternatives
		...
		while ($n <= $necho)
			 /data/nil-bluearc/GMT/Laumann/NEW_PROC_TEST/one_step_resampling_AT_TL.csh -i bold$runID[$k]/$patid"_b"$runID[$k]_echo${n}_faln -xr3dmat $xr3dmat \
			-phase ${PHA_on_EPI}_faln_xr3d -ped $ped -dwell $dwell -ref $outspace $strwarp $parallelstr \
	...
	echo	/data/nil-bluearc/GMT/Laumann/NEW_PROC_TEST/one_step_resampling_AT_TL.csh -i bold$runID[$k]/$patid"_b"$runID[$k]_echo?_faln.4dfp.img -xr3dmat $xr3dmat \
			-phase ${PHA_on_EPI}_faln_xr3d -ped $ped -dwell $dwell -ref $outspace $strwarp $parallelstr \
			-trailer xr3d_uwrp_on_${outspacestr}
		/data/nil-bluearc/GMT/Laumann/NEW_PROC_TEST/one_step_resampling_AT_TL.csh -i bold$runID[$k]/$patid"_b"$runID[$k]_echo?_faln.4dfp.img -xr3dmat $xr3dmat \
			-phase ${PHA_on_EPI}_faln_xr3d -ped $ped -dwell $dwell -ref $outspace $strwarp $parallelstr \
...
###############################
#set C = /home/usr/shimonyj/me_fmri	# MEfmri_4dfp source code directory
if (! ${?ME_reg}) @ ME_reg = 1
@ k = 1
while ($k <= $runs)
	pushd bold$runID[$k]
