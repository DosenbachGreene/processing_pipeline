#!/bin/csh -f
#$Header: processing_pipeline/me_pipeline/scripts/bin/SE_cross_bold_pp.csh,v 1.23 2023/09/07 $
#$Log: SE_cross_bold_pp.csh,v $
...
# Initial revision
#
set idstr = '$Id: SE_cross_bold_pp.csh$'
echo $idstr
...
@ k = 1
while ($k <= $runs)
	rm -f bold$runID[$k]/${patid}"_b"${runID[$k]}_echo1_xr3d.mat	# force cross_realign3d_4dfp to recompute
...
@ k = 1
while ($k <= $runs)
	mat2dat bold$runID[$k]/${patid}_b${runID[$k]}_echo1_xr3d.mat -RD -n$skip || exit $status
	/bin/mv bold$runID[$k]/${patid}_b${runID[$k]}_echo1_xr3d.*dat movement
	@ k++
end
...
#########################
	...
	@ k = 1
	while ($k <= $runs)
		echo bold$runID[$k]/$patid"_b"$runID[$k]"_echo1"${MBstr} mat=bold$runID[$k]/$patid"_b"$runID[$k]_echo1_xr3d.mat \
			>> $patid"_xr3d".lst
		@ k++
	end
	cat $patid"_xr3d".lst
...
#########################
	...
	while ($k <= $runs)
		pushd bold$runID[$k]	# compute bias field for each run
			set n = `cat $patid"_b"$runID[$k]"_echo1"${MBstr}_xr3d.4dfp.ifh | grep "matrix size \[4\]" | gawk '{print $NF-1}'`
			actmapf_4dfp x${n}+ $patid"_b"$runID[$k]"_echo1"${MBstr}_xr3d -aavg || exit $status
			set base = ${patid}"_b"$runID[$k]"_echo1"${MBstr}_xr3d_avg
			...
			imgopr_4dfp -p$patid"_b"$runID[$k]"_echo1"${MBstr}_xr3d_BC $patid"_b"$runID[$k]"_echo1"${MBstr}_xr3d \
...
while ( $i <= $#BOLDgrps )
	...
	######################################
	grep -x -A4 "t4 frame 1" bold${run}/$patid"_b"${run}_echo1_xr3d.mat | tail -4 >  $adir/bold${run}_tmp.mat
	############################
	# tmp intensity scale factor
	############################
	grep -x -A6 "t4 frame 1" bold${run}/$patid"_b"${run}_echo1_xr3d.mat | tail -1 >> $adir/bold${run}_tmp.mat
	##################################################################
	...
	while ( $j <= $#runs )
		if ( $OneStepResample ) then
			if ($old_faln_xr3d) then
				set xr3dmat = bold$runID[$k]/$patid"_b"$runID[$k]_echo1_xr3d.mat
			else
				set xr3dmat = bold$runID[$k]/$patid"_b"$runID[$k]_echo1${MBstr}_xr3d.mat
			endif
			if ($BiasField) then
				set OneStepstr = "-bias bold$runID[$k]/${patid}"_b"$runID[$k]_echo1${MBstr}_xr3d_avg_BF"
			else
				set OneStepstr = ""
			endif
