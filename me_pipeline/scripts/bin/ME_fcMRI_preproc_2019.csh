...
###################################
	...
	@ k = 1
	while ($k <= $runs)
		set file = bold$FCrunID[$k]/$patid"_b"$FCrunID[$k]_faln_xr3d_uwrp_on_${outspacestr}_Swgt_norm
		echo $file >> $$.lst
		@ k++
	end
...
echo	run_dvar_4dfp $conc -m${patid1}_FSWB_on_${outspacestr} -n$skip $xstr -b$DVARblur -T$DVARsd -S
	run_dvar_4dfp $conc -m${patid1}_FSWB_on_${outspacestr} -n$skip $xstr -b$DVARblur -T$DVARsd -S || exit $status
...
source ../bold$FCrunID[1]/${patid}_b$FCrunID[1].params	# define $TR_vol and $ped (same for all runs)
if (! ${?lomotil}) then
	set lmstr = "";
else
...
while ($k <= $runs)
	...
	mat2dat -D -n$skip        $patid"_b"$FCrunID[$k]_xr3d.mat $lmstr		|| exit -1
	gawk -f $RELEASE/FD.awk   $patid"_b"$FCrunID[$k]_xr3d.ddat >> ${patid}_xr3d.FD	|| exit -1
	@ k++
end
rm $patid*.mat $patid*.dat $patid*.ddat
if ($?FDthresh) then 
	if (! ${?FDtype}) then
		@ FDtype = 1
...
@ k = 1
while ($k <= $runs)
	set root = ../bold$FCrunID[$k]/${patid}_b$FCrunID[$k]_xr3d
	mat2dat    $root -I
	set file = ${root}_dat			
	echo $file >> $$.lst
	@ k++
...
/bin/rm $$.lst
source ../bold$FCrunID[1]/${patid}_b$FCrunID[1].params	# define $TR_vol (same for all runs)
